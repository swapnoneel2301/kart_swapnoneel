...
class Search extends React.Component {
   ...
   */
  getProducts = async () => {
    let resp = await this.performAPICall();
  ...
  componentDidMount(){
     this.getProducts();
     this.setState({loggedIn : localStorage.getItem('username')?true:false});
  ...
  search = (text) => {
    ...
    elements = elements.filter((ele)=>{
      ...
      return name.includes(search_text) || category.includes(search_text);
    });
    // console.log('from search',text);
    this.setState({filteredProducts:elements});
