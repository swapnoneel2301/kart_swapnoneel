...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response, "delete this address")) {
      if (response) {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        let total_cost = this.cartRef.current.calculateTotal();
        localStorage.setItem('balance',this.state.balance - total_cost);
  ...
  if(total_cost>this.state.balance){
    message.error('Does not have sufficient balance');
    return;
  }
  if(this.state.addresses.length===0){
    message.error('Give your address');
    return;
  }
  if(this.state.selectedAddressIndex === undefined){
    message.error('Select your address');
    return;
  }
    await this.checkout();
  };
  ...
  async componentDidMount() {
     ...
     if(localStorage.getItem('username') && localStorage.getItem('token')){
      ...
      // protect the route if no items in the cart
       await fetch(`${config.endpoint}/cart`, {
          ...
          .then(products=>{
            if(products.length===0){
             message.error('You must add items to the cart first');
      ...
      // updating products
       await fetch(`${config.endpoint}/products`, {
            method: "GET",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
       ...
       message.error('User must logged In');
